...
class SamePadConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, groups=1, gamma=0.9):
        ...
        # split the gradient into equal size chunks
    ...
    def store_grad(self):
        ...
        grad = grad.view(-1)
        self.f_grads = self.f_gamma * self.f_grads + (1-self.f_gamma) * grad  # f_gamma = 0.3 -> fast-learn
        ...
        self.grads = self.gamma * self.grads + (1-self.gamma) * grad  # gamma = 0.9 -> slow-learn
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, final=False, gamma=0.9, groups=1):
        super().__init__()
        self.conv1 = SamePadConv(in_channels, out_channels, kernel_size, dilation=dilation, gamma=gamma, groups=groups)
        self.conv2 = SamePadConv(out_channels, out_channels, kernel_size, dilation=dilation, gamma=gamma, groups=groups)
...
class DilatedConvEncoder(nn.Module):
    def __init__(self, in_channels, channels, kernel_size, gamma=0.9, groups=1):
        super().__init__()
