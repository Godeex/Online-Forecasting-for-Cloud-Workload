...
from models.ts2vec.fsnet_encoder import TSEncoder
...
class net(nn.Module):
    def __init__(self, args, device):
        ...
        self.time_index = 3
        encoder_time = TSEncoder(input_dims=args.seq_len,
                            output_dims=320,  # standard ts2vec backbone value
                            hidden_dims=64, # standard ts2vec backbone value
                            depth=10)
        
        self.encoder_time = TS2VecEncoderWrapper(encoder_time, mask='all_true').to(self.device)
        self.regressor_time = nn.Linear(320, args.pred_len).to(self.device)

        encoder = TSEncoder(input_dims=args.enc_in + self.time_index,
                            output_dims=320,  # standard ts2vec backbone value
                            hidden_dims=64, # standard ts2vec backbone value
                            depth=10)
    ...
    def forward_individual(self, x, x_mark):
        # y1
        rep1 = self.encoder_time.encoder.forward_time(x)
        y1 = self.regressor_time(rep1).transpose(1, 2)
        y1 = rearrange(y1, 'b t d -> b (t d)')
        # y2
        x2 = torch.cat([x, x_mark], dim=-1)
        rep2 = self.encoder(x2)[:, -1]
    ...
    def forward_weight(self, x, x_mark, g1, g2):
        ...
        y1 = self.regressor_time(rep1).transpose(1, 2)
        y1 = rearrange(y1, 'b t d -> b (t d)')
        # print("y1.detach() * g1:", (y1.detach() * g1).size())
        """
        """
        x2 = torch.cat([x, x_mark], dim=-1)
        rep2 = self.encoder(x2)[:, -1]
        y2 = self.regressor(rep2)
        # print("y2.detach() * g2:", (y2.detach() * g2).size())
...
class Exp_TS2VecSupervised(Exp_Basic):
    ...
    def train(self, setting):
        ...
        self.opt_weight = optim.AdamW([self.weight], lr=5e-4)
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(vali_loader):
            pred, true, outputs = self._process_one_batch(
                vali_data, batch_x, batch_y, batch_x_mark, batch_y_mark, mode='vali')
            pred = pred[0] * 0.5 + pred[1] * 0.5
            # pred = outputs
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, mode='train'):
        ...
        loss_weight = criterion(outputs, true)
        loss_weight.backward()
        self.opt_weight.step()
        self.opt_weight.zero_grad()
        # print("loss1:", loss1)

        return [y1, y2], true, outputs
