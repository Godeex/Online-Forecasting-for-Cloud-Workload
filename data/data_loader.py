...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None, 
                 features='S', data_path='ETTh1.csv', 
                 target='OT', scale=True, inverse=False, timeenc=2, freq='t', cols=None):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24*4*4
    ...
    def __getitem__(self, index):
        ...
        r_begin = s_end - self.label_len 
        r_end = r_begin + self.label_len + self.pred_len
        # size [seq_len, label_len, pred_len] [60, 0, 1->72]
        seq_x = self.data_x[s_begin:s_end]
        if self.inverse:
            seq_y = np.concatenate([self.data_x[r_begin:r_begin+self.label_len], self.data_y[r_begin+self.label_len:r_end]], 0)
        else:
            seq_y = self.data_y[r_begin:r_end]
        seq_x_mark = self.data_stamp[s_begin:s_end] # [seq_len = 60]
        seq_y_mark = self.data_stamp[r_begin:r_end] # [self.label_len + self.pred_len = 1]
